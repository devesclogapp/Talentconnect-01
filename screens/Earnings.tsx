  ...
  return (
    <div className="min-h-screen pb-32 transition-colors animate-fade-in bg-app-bg">
      <header className="safe-area-top sticky top-0 z-40 bg-white dark:bg-black border-b border-neutral-100 dark:border-neutral-900 px-4 py-4 flex items-center justify-between">
        ...
        <h2 className="meta-bold tracking-[0.2em] text-xs">Meus ganhos</h2>
      ...
      <div className="py-6 space-y-6">
        ...
        <section className="px-4">
          <div className="relative overflow-hidden rounded-sm p-8 text-white bg-black border border-neutral-800">
            ...
            <div className="relative z-10">
              <p className="meta-bold tracking-[0.2em] opacity-40 !text-xs flex items-center gap-2 mb-3">
              ...
              <button className="w-full h-14 bg-primary-orange text-white rounded-none meta-bold tracking-[0.2em] !text-xs active:scale-95 transition-all shadow-xl shadow-primary-orange/20 flex items-center justify-center gap-2">
                <Wallet size={16} />
        ...
        <section className="px-4">
          <div className="bg-white dark:bg-black rounded-sm p-6 border border-neutral-100 dark:border-neutral-900">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="meta-bold text-text-primary tracking-[0.2em] !text-xs mb-1">Performance</h3>
                <p className="meta-bold text-text-tertiary !text-xs tracking-widest">Últimos 7 dias</p>
              </div>
            ...
            <div className="h-40 w-full mb-6">
              {loading ? (
                <div className="h-full flex items-center justify-center meta text-text-primary tracking-widest !text-xs">Analisando...</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    <Tooltip
                      ...
                      labelStyle={{ color: 'var(--text-tertiary)', fontSize: '12px', marginBottom: '4px' }}
        ...
        {completedOrders.length > 0 && (
          <section className="px-4 pb-10">
            <h3 className="meta-bold text-neutral-300 tracking-[0.2em] !text-xs mb-4">Registro histórico</h3>
                  ...
                  >
                    <div className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="meta-bold tracking-tight !text-xs text-text-primary">
                            {order.service?.title || 'Serviço'}
                          </p>
                          <p className="meta !text-xs text-text-secondary flex items-center gap-1 tracking-widest mt-0.5">
                            <Clock size={10} />
                      ...
                      <div className="text-right">
                        ...
                        {isExpanded && <p className="meta !text-xs tracking-widest text-text-tertiary">Líquido</p>}
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 bg-neutral-50/50 dark:bg-neutral-900/30 border-t border-neutral-100 dark:border-neutral-800 animate-slide-down">
                        <div className="pt-4 grid grid-cols-3 gap-4 text-center">
                          <div>
                            <p className="meta !text-xs text-text-tertiary tracking-widest mb-1">Valor bruto</p>
                            <p className="font-bold text-text-primary text-xs">R$ {gross.toFixed(2)}</p>
                          </div>
                          <div>
                            <p className="meta !text-xs text-text-tertiary tracking-widest mb-1">Taxa operadora</p>
                            <p className="font-bold text-error text-xs">- R$ {fee.toFixed(2)}</p>
                          </div>
                          <div>
                            <p className="meta !text-xs text-text-tertiary tracking-widest mb-1">Seu ganho</p>
                        ...
                        <div className="mt-4 pt-3 border-t border-dashed border-neutral-200 dark:border-neutral-800 flex justify-between items-center">
                          <span className="meta !text-xs text-text-tertiary tracking-widest">ID: #{order.id.slice(0, 8)}</span>
                          <button className="text-xs font-bold text-primary-orange tracking-widest hover:underline">Ver recibo</button>
                        </div>
                      </div>
                    )}
                  </div>
...
const StatBlock = ({ icon, value, label, color }: any) => (
  <div className="bg-neutral-50 dark:bg-neutral-950 rounded-sm p-4 border border-neutral-100 dark:border-neutral-900">
    ...
    <p className="meta-bold text-text-secondary tracking-widest !text-xs">{label}</p>
  </div>
);
